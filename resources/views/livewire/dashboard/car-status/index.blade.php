...
@section('tableName', __('Car Status'))
            ...
            <tr>
                ...
                <td>
                    <button type="button" class="btn btn-outline-primary mb-2" data-bs-toggle="modal"
                        data-bs-target="#editModal" wire:click.prevent="edit({{ $data->car_status_id }})"><i
...
@section('create-model-body')
    @if (Session()->has('WrongCarStatus'))
        <div class="alert alert-danger">
            {{ Session('WrongCarStatus') }}
        </div>
    @endif
    @if (!Session()->has('WrongCarStatus'))
        <div class="card-body">
...
@section('edit-model-body')
    ...
    @if (!Session()->has('WrongCarStatus'))
        <div class="card-body">
            <form autocomplete="off" method="POST">
                @csrf
                <input type="hidden" name="car_status_id" wire:model="car_status_id">
                <div class="results">
                    @if (Session()->has('message'))
                        <div class="alert alert-success">
                            {{ Session('message') }}

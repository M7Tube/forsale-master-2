...
@section('tableName', __('Ad Status'))
            ...
            <tr>
                ...
                <td>
                    <button type="button" class="btn btn-outline-primary mb-2" data-bs-toggle="modal"
                        data-bs-target="#editModal" wire:click.prevent="edit({{ $data->ad_statuse_id }})"><i
...
@section('create-model-body')
    @if (Session()->has('WrongAdStatus'))
        <div class="alert alert-danger">
            {{ Session('WrongAdStatus') }}
        </div>
    @endif
    @if (!Session()->has('WrongAdStatus'))
        <div class="card-body">
...
@section('edit-model-body')
    ...
    @if (!Session()->has('WrongAdStatus'))
        <div class="card-body">
            <form autocomplete="off" method="POST">
                @csrf
                <input type="hidden" name="ad_statuse_id" wire:model="ad_statuse_id">
                <div class="results">
                    @if (Session()->has('message'))
                        <div class="alert alert-success">
                            {{ Session('message') }}

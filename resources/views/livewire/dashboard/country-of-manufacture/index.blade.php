...
@section('tableName', __('Country Of Manufacture'))
            ...
            <tr>
                ...
                <td>
                    <button type="button" class="btn btn-outline-primary mb-2" data-bs-toggle="modal"
                        data-bs-target="#editModal" wire:click.prevent="edit({{ $data->cof_id }})"><i
...
@section('create-model-body')
    @if (Session()->has('WrongCountryOfManufacture'))
        <div class="alert alert-danger">
            {{ Session('WrongCountryOfManufacture') }}
        </div>
    @endif
    @if (!Session()->has('WrongCountryOfManufacture'))
        <div class="card-body">
...
@section('edit-model-body')
    ...
    @if (!Session()->has('WrongCountryOfManufacture'))
        <div class="card-body">
            <form autocomplete="off" method="POST">
                @csrf
                <input type="hidden" name="cof_id" wire:model="cof_id">
                <div class="results">
                    @if (Session()->has('message'))
                        <div class="alert alert-success">
                            {{ Session('message') }}

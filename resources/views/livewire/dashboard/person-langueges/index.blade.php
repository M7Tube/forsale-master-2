...
@section('tableName', __('Person Langueges'))
            ...
            <tr>
                ...
                <td>
                    <button type="button" class="btn btn-outline-primary mb-2" data-bs-toggle="modal"
                        data-bs-target="#editModal" wire:click.prevent="edit({{ $data->lang_id }})"><i
...
@section('create-model-body')
    @if (Session()->has('WrongPersonLangueges'))
        <div class="alert alert-danger">
            {{ Session('WrongPersonLangueges') }}
        </div>
    @endif
    @if (!Session()->has('WrongPersonLangueges'))
        <div class="card-body">
...
@section('edit-model-body')
    ...
    @if (!Session()->has('WrongPersonLangueges'))
        <div class="card-body">
            <form autocomplete="off" method="POST">
                @csrf
                <input type="hidden" name="lang_id" wire:model="lang_id">
                <div class="results">
                    @if (Session()->has('message'))
                        <div class="alert alert-success">
                            {{ Session('message') }}

            ...
            <tr>
                ...
                <td>
                    <button type="button" class="btn btn-outline-primary mb-2" data-bs-toggle="modal"
                        data-bs-target="#editModal" wire:click.prevent="edit({{ $data->car_type_id }})"><i
...
@section('create-model-body')
    @if (Session()->has('WrongCarType'))
        <div class="alert alert-danger">
            {{ Session('WrongCarType') }}
        </div>
    @endif
    @if (!Session()->has('WrongCarType'))
        <div class="card-body">
...
@section('edit-model-body')
    ...
    @if (!Session()->has('WrongCarType'))
        <div class="card-body">
            <form autocomplete="off" method="POST">
                @csrf
                <input type="hidden" name="car_type_id" wire:model="car_type_id">
                <div class="results">
                    @if (Session()->has('message'))
                        <div class="alert alert-success">
                            {{ Session('message') }}
